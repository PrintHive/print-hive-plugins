...
description: >-
  ...
  or when resolving material UUIDs to names via catalog name search or Spool.
---
...
Resolve Make material UUIDs via catalog **name** search (`types: ["material"]`) or Spool. Never HEX fishing or UUID-as-search.
...
- On catalog miss at match time, re-call `makes_list` / `jobs_get` after queue or start. Makes are often created at start time (create-at-start lag); Counter's earlier pass can miss them.
...
When a Make has `required_materials`, stage and speak catalog **brand + name** first. Use HEX only if names are missing. If UUID→name resolve fails, hand off to Spool (or say resolve via catalog name search) — do **not** speak slice HEX as the material identity.
